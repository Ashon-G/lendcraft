...
Other than that: If you have any questions send an email to [support@arcadekit.dev].
...
You'll only want it, if you have something to render in the 'Preview Box' on your Product's Product Page. For reference, go to `https://arcadekit.dev/components/quest-card` and check the 'Preview' section in which the quest card is actually being rendered. The source code of this component would be placed in the `quest-card.tsx` file.
